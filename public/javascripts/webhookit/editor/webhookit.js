...
var webhookit = {
   ...
   language: {
      ...
      {"type": "string", inputParams: {"name": "tag", label: "Tag", typeInvite: "Enter a tag" } },
			{"type": "text", inputParams: {"name": "description", label: "Description", cols: 30} }
		],
		
		layoutOptions: {
		 	units: ([{ position: 'bottom', height: 170, body: 'bottom', resize: true, header: '<button id="refreshDebug">Debug</button><button id="refreshXML">XML</button>', scroll: true, collapse: true, gutter: '5px'}]).concat(WireIt.BaseEditor.defaultOptions.layoutOptions.units)
		},
   ...
   run: function() {
		...
		window.location = "/topologies/"+prev.id;
   },
