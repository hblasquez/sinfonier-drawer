 ...
 /**
  ...
  /**
	 ...
	 save: function() {
      ...
      this.tempSavedWiring = {prev:previous,
  ...
  /**
    ...
    */
      ...
      var zoomOutButton = new YAHOO.widget.Button({ label:"Zoom out", id:"WiringEditor-zoomOutButton", container: toolbar });
  ...
  addModule: function(module, pos) {
    zoomPos = pos.map(function(num){
      return num / this.scale;
    },this);
    webhookit.WiringEditor.superclass.addModule.call(this,module,zoomPos);
  },
  ...
  loadPipe: function(name) {
    ...
    try {
      ...
      if (wiring.properties.name) // When loading a template not set readonly
      ...
      if(YAHOO.lang.isArray(wiring.modules)) {
        ...
        for(i = 0 ; i < wiring.modules.length ; i++) {
          ...
          if(!this.modulesByName[m.name]) {
            if (!searched[m.name]) {
              notFound.push(this.adapter.loadModule(m.name,this.modulesByName));
              searched[m.name] = true;
            }
          }
        }
        var self = this;
        $.when.apply(this.adapter,notFound).done( function(){
            // Containers
            for(i = 0 ; i < wiring.modules.length ; i++) {
              var m = wiring.modules[i];
              if(self.modulesByName[m.name]) {
                var baseContainerConfig = self.modulesByName[m.name].container;
            ...
            if(YAHOO.lang.isArray(wiring.wires)) {
              for(i = 0 ; i < wiring.wires.length ; i++) {
                ...
                self.layer.addWire(wiring.wires[i]);
              }
            }
