...
var prepareClicks = function(){
  ...
  $(".topologies-diagram-list").each(function(index) {
    var $diagram = $(this);
    ...
    if ($diagram.find('canvas').length == 0) {
      ...
      var topologyDescription = JSON.parse($desc.html());
      drawTopology(ctxt, topologyDescription, totalWidth, totalHeight);
      //$desc.remove();
    }
  });
  $(".topologies-diagram-list").mouseenter(function(){
    var $diagram = $(this);
    var $desc = $diagram.find(".topology-description");
...
*/
  var $container = $('#topologies-list');

  if ($container.infinitescroll) {
    $container.infinitescroll({
          navSelector: '#page-nav',    // selector for the paged navigation
          nextSelector: '#page-nav a',  // selector for the NEXT link (to page 2)
          itemSelector: '.boxdetail',     // selector for all items you'll retrieve
          loading: {
        ...
        function (newElements) {
          ...
          $newElems.find(".log-expanded").toggle(false);
