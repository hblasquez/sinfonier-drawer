...
YAHOO.lang.extend(WireIt.FormContainer, WireIt.Container, {
   ...
   getValue: function() {
     ...
     if (this.parallEl) {
       value.parallelism = this.parallEl.value;
     }
     return value;
   },
   ...
   setValue: function(val) {
      if (val && val.parallelism)
      {
        if (this.parallEl)
        {
          this.parallEl.value = String(val.parallelism);
          this.el.style.boxShadow = this.calcShadow(val.parallelism);
        }
      }
